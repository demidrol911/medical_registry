...
class DayHospitalAll(MedicalServiceTypePage):
    ...
    def get_query(self):
        query = MedicalServiceTypePage.get_general_query() + '''
                SELECT
                    ...
                    COUNT(DISTINCT (patient_id, md.term_fk, service_tariff_profile)) AS count_patients,
                    COUNT(DISTINCT CASE WHEN is_adult
                                          THEN (patient_id, md.term_fk, service_tariff_profile)
                ...
                WHERE (service_term = 2 AND service_group IS NULL AND is_regional_budget)
                      OR service_group IN (28, 30, 17)
