...
class ClinicEmergencySpec(MedicalServiceTypePage):
        ...
        query = MedicalServiceTypePage.get_general_query() + '''
                ...
                WHERE
                     ((service_term = 3 AND service_reason = 5 AND (service_group = 24 OR service_group IS NULL))
                      ...
                      AND (service_division NOT IN (399, 401, 403) OR service_division IS NULL)
...
class EmergencyCareEmergencyDepartment(MedicalServiceTypePage):
...
class ClinicEmergencyAll(MedicalServiceTypePage):
