...
class HospitalAmbulancePage(MedicalServiceTypePage):
    """
    Лист принятых услуг в
    приемном отделении стационара (неотложная форма)
    """
...
class AmbulanceVisitNursingStaffPage(MedicalServiceTypePage):
    """
    Лист принятых услуг по
    1. Поликлиника (в неотложной форме, посещения к среднему мед.персоналу)
    """
        ...
        query = MedicalServiceTypePage.get_general_query() + '''
                SELECT
                    ...
                    SUM(CASE WHEN NOT is_adult
                        ...
                        END) AS total_tariff_child
                FROM registry_services
                WHERE service_code IN ('056067', '156067')
                GROUP BY mo_code, group_field
                '''
        return query
