...
class IntegratedPrenatalDiagnosisPage(MedicalServiceTypePage):
    """
    Лист принятых услуг по
    комплексной пренатальной (дороовой) диагностики при сроке беременности 11-14 недель
    """
    ...
    def get_query(self):
        query = MedicalServiceTypePage.get_general_query() + '''
                SELECT
                    mo_code AS mo_code,
                    CASE WHEN service_code IN ('001840', '101840') THEN '0'
                         WHEN service_code IN ('001842', '101842') THEN '1'
                    END AS group_field,
                ...
                WHERE service_code IN ('001840', '101840', '001842', '101842')
...
class ScreeningUltrasoundPage(MedicalServiceTypePage):
    """
    Лист принятых услуг по
    скрининговому ультразвуковому исследованию при сроке беременности 18-21 неделя
    """
