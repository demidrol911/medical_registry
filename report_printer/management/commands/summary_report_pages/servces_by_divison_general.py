...
from report_printer.libs.excel_style import (
    VALUE_STYLE,
    TITLE_STYLE,
    TOTAL_STYLE
)
...
class GeneralServicesPage(ReportPage):
    ...
    def _construct_query(self):
        ...
        WITH all_data AS (
            SELECT
                            ...
                            WHEN ms.group_fk = 7
                              THEN (
                                CASE ms.code
                                    ...
                                    ELSE 'Прочие услуги (исследования, анализы)'
                                END)
                ...
                CASE
                    WHEN (pe.term_fk = 3 AND ps.payment_kind_fk = 2) or (pe.term_fk = 4 AND ps.payment_kind_fk = 2)
                      THEN True
                    ELSE False
                ...
                ROUND(CASE WHEN tc.id_pk in (8, 9, 26) THEN tc.value*ps.tariff ELSE 0 END, 2) AS cpg_coefficient,
    ...
    def _convert_capitation(self, capitation_data):
        convert_capitation = self._reset_total()
        convert_capitation['patients_child'] = capitation_data['child']['population']
        convert_capitation['quantities_child'] = capitation_data['child']['basic_tariff']
        convert_capitation['tariffs_child'] = capitation_data['child']['tariff']
        convert_capitation['fap_coefficient_child'] = capitation_data['child']['coeff']
        convert_capitation['accepted_payment_child'] = capitation_data['child']['accepted']

        convert_capitation['patients_adult'] = capitation_data['adult']['population']
        convert_capitation['quantities_adult'] = capitation_data['adult']['basic_tariff']
        convert_capitation['tariffs_adult'] = capitation_data['adult']['tariff']
        convert_capitation['fap_coefficient_adult'] = capitation_data['adult']['coeff']
        convert_capitation['accepted_payment_adult'] = capitation_data['adult']['accepted']
    ...
    def print_page(self, sheet, parameters):
        ...
        total_capitation = self._reset_total()
        totals_by_term = {
            ...
            u'Диспансеризация взрослых': {u'on': False, u'total': self._reset_total()},
        ...
        if self.policlinic_capitation or self.ambulance_capitation:
            if self.policlinic_capitation:
                self._accumulate_total(self.policlinic_capitation, total_capitation)
            if self.ambulance_capitation:
                self._accumulate_total(self.ambulance_capitation, total_capitation)
            self._accumulate_total(total_capitation, total_by_mo)
            sheet.set_style(TITLE_STYLE)
            sheet.write(u'Подушевой норматив', 'r', self.COUNT_CELL_IN_ACT)
    ...
    def _print_capitation(self, sheet, title, capitation_data):
        if capitation_data:
            sheet.set_style(VALUE_STYLE)
            sheet.write(title, 'c')
            self._print_item(sheet, capitation_data)
