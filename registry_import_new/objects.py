...
class RegistryDb:
    ...
    def create_patient_obj(self, patient):
        patient_obj = Patient(
            ...
            last_name=patient.get('FAM', '').upper(),
            first_name=patient.get('IM', '').upper(),
            ...
            person_id_series=patient.get('DOCSER', ''),
            person_id_number=patient.get('DOCNUM', ''),
            weight=safe_float(patient.get('VNOV_D', 0)),
            okato_residence=patient.get('OKATOP', ''),
            okato_registration=patient.get('OKATOG', ''),
            comment=patient.get('COMENTP', ''),
            agent_last_name=patient.get('FAM_P', '').upper(),
            agent_first_name=patient.get('IM_P', '').upper(),
    ...
    def patient_update_policy(self, patient_obj, patient_policy):
        ...
        if idx >= 0:
            ...
            self.patients[idx].insurance_policy_series = patient_policy.get('SPOLIS', '')
            self.patients[idx].insurance_policy_number = patient_policy.get('NPOLIS', '')
            self.patients[idx].newborn_code = patient_policy.get('NOVOR', '')

    def create_registry_object(self, registry, registry_item):
        invoice = dict(id=registry['NSCHET'], date=registry['DSCHET'])
