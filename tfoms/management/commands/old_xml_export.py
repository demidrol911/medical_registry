...
def get_records(register_pk):
    query = """
        select
            ...
            (
                select DISTINCT medical_error.failure_cause_fk
                from provided_service ps2
        ...
        where
            medical_register.is_active
            and medical_register.year = '2014'
            and medical_register.period = '01'
...
def main():
    ...
    for index, register in enumerate(registers):
        ...
        records = list(get_records(register.organization_code))
        new_register = True
        current_record = None
        current_event = None
        event_counter = 0
        ...
        for i, record in enumerate(records):
            ...
            if record.event_uid != current_event:
                ...
                if record.event_payment_type_code in (2, 3):
                    hm_xml.put('REFREASON', record.event_sanctions or '')
                else:
                    hm_xml.put('REFREASON', '')
                hm_xml.put('SANK_MEK', record.event_sanctions_mek or '')
            ...
            sumv_usl = round(float(record.service_invoiced_payment), 2)
            sump_usl = round(float(record.service_accepted_payment), 2)
            hm_xml.put('SUMV_USL', sumv_usl or 0)
            hm_xml.put('OPLATA', record.service_payment_type_code or '')
            hm_xml.put('SUMP_USL', sump_usl or 0)
            if record.service_payment_type_code in (2, 3):
                hm_xml.put('REFREASON', record.service_failure_cause_code or '')
            else:
                hm_xml.put('REFREASON', '')
            hm_xml.put('SANK_MEK', record.service_sanction_mek or 0)
